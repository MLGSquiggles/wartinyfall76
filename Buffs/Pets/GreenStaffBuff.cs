...
namespace wartinyfall76.Buffs.Pets
{
	public class GreenStaffBuff : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex)
		{
			...
			bool petProjectileNotSpawned = player.ownedProjectileCounts[mod.ProjectileType("GreenStaffProjectile")] <= 0;
			if(petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
			{
				Projectile.NewProjectile(player.position.X + (float)(player.width * 0.5), player.position.Y +(float)(player.height * 0.5), 0f, 0f, mod.ProjectileType("GreenStaffProjectile"), 0, 0f, player.whoAmI, 0f,0f);
			}
		}
